...
public class WorkingStackView extends StackPane implements GameModelListener
{
    ...
    private static Workingstack aIndex;
    ...
    private void buildLayout()
    {
        ...
        int offset = 0;
        Card[] stack = GameModel.getInstance().getWorkingStack(aIndex);
...
//                    if( GameModel.getInstance().isLegalMove(transfer.getTop(), aIndex) )
//                    {
    ...
    private EventHandler<DragEvent> createDragDroppedHandler(final ImageView pImageView, final Card pCard)
    {
        return new EventHandler<DragEvent>() 
        {
            public void handle(DragEvent pEvent)
            {
                ...
                if(db.hasString()) 
                {
//                    GameModel.getInstance().getCardMove(new CardTransfer(db.getString()).getTop(), aIndex).perform(); 
                    success = true;
                }
